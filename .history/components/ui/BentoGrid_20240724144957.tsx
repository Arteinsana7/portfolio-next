...
export const BentoGridItem = ({
  ...
  const leftLists = ["Postgre", "HTML", "JavaScript", "React"];
      ...
      <div className="relative w-full h-full">
        {img && (
          <img
            src={img}
            alt={img}
            className={`absolute w-full h-full object-cover object-center ${id === 5 ? "translate-y-[-2rem]" : ""}`}
          />
        )}
        {spareImg && (
          <div className={`absolute right-0 ${id === 5 ? "bottom-0" : "-bottom-5"} w-full h-full`}>
            <img
              src={spareImg}
              alt={spareImg}
        ...
        {id === 6 && (
          <BackgroundGradientAnimation>
            <div className="absolute inset-0 flex items-center justify-center text-white font-bold px-4 pointer-events-none text-3xl text-center md:text-4xl lg:text-7xl"></div>
          </BackgroundGradientAnimation>
        )}
        <div
          className={cn(
        ...
        >
          <div className="font-sans text-lg lg:text-3xl max-w-96 font-medium z-10">
            {title}
          </div>
